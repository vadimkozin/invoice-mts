...
export const drawHeader = ({ doc, cfg, data }) => {
    ...
    const [addressOperator, addressCustomer, countRowAddress] = calculateCountRows({ doc, str1: data.operator.address, str2: data.customer.address, wMax1: 100, wMax2: 85, fontSize: 8 });
    const addingAddress = (countRowAddress - 1) * cfg.stepHeaderY;
    const operator = `${data.operator.name} (${data.operator.nameShort})`;
    const [nameOperator, nameCustomer, countRowName] = calculateCountRows({ doc, str1: operator, str2: data.customer.name, wMax1: 100, wMax2: 85, fontSize: 10 });

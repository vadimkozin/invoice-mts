/**
 * Cумма прописью для чисел от 0 до 999 триллионов
 * @param {*} cost
 * @param {*} currency: RUB,USD,EUR (по умолчанию RUB)
 * @returns Сумма прописью
 */
export const getCostInWords = (cost, currency = 'RUB') => {
    const x = Number(cost.toFixed(2));
        ...
        for (i = 4; i >= 0; i--) {
            if (t[i] > 0) {
                ...
                if (names[d[i][11]])
                    ...
                    if (names[d[i][0]])
                        r += ' ' + (typeof names[d[i][0]] == 'object' ? names[d[i][0]][i] : names[d[i][0]]);
                }
                if (names[d[i][11]])
    ...
    y = Number(((x - Math.floor(x)) * 100).toFixed(2));
    if (y < 10)
        y = '0' + y;
